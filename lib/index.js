...
function useViewportRows({ element, rows, rowHeight, clientHeight, scrollTop, enableVirtualization, gridHeight }) {
	const { totalRowHeight, gridTemplateRows, getRowTop, getRowHeight, findRowIdx } = useMemo(() => {
		...
		if (typeof rowHeight === "string") {
			const _headerElement = (element$1) => element$1.querySelector("[role=\"columnheader\"]");
			const _rowElement = (element$1, rowIdx) => {
				const nth = _headerElement(element$1) ? rowIdx + 1 : rowIdx;
				return element$1.querySelector(`[role="row"]:nth-of-type(${nth})`);
			};
			return {
	...
	function getNextPosition(key, ctrlKey, shiftKey) {
		...
		switch (key) {
			...
			case "PageUp": {
				...
				const rowHeight$1 = getRowHeight(rowIdx);
				const nextRowY = rowTop + rowHeight$1 - clientHeight;
				const nextRowIdx = nextRowY > 0 ? findRowIdx(nextRowY) : 0;
				return {
					idx,
					rowIdx: rowTop === -1 || rowHeight$1 === -1 || nextRowIdx === -1 ? rowIdx + 1 : nextRowIdx
				};
			}
			case "PageDown": {
				...
				return {
					idx,
					rowIdx: rowTop === -1 || nextRowIdx === -1 ? rowIdx - 1 : nextRowIdx
				};
			}
